...
tokenizer = AutoTokenizer.from_pretrained(model_name, truncation=True, max_length=2056)
...
# Ensure LoRA layers are in bfloat16 or float32 precision
for name, module in model.named_modules():
...
def tokenize_data(example):
    ...
    inputs = tokenizer(input_text, return_tensors="pt", padding="max_length", truncation=True, max_length=2056)
    outputs = tokenizer(output_text, return_tensors="pt", padding="max_length", truncation=True, max_length=512)
